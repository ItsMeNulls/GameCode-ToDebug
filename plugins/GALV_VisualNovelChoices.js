...
(function () {
	...
	Window_ChoiceList.prototype.itemRectForText = function (index) {
		...
		if ($gameSystem.vnChoices) {
			...
			// count icon code
			var icons = txt.match(/\\i\[\d+\]/gi);
			icons = icons ? icons.length * (Window_Base._iconWidth + 4) : 0;

			txt = this.convertEscapeCharacters(txt, index);
			txt = txt.replace(/\x1bi\[\d+\]/gi, "");
			txt = txt.replace(/\x1bc\[\d+\]/gi, "");
			txt = txt.replace(/\x1bC/gi, "");
			txt = txt.replace(/+/g, "");		
