...
//=============================================================================
/*:
 * @plugindesc 1.8.2 (Updated by DrDhoom for MV 1.6.1) Custom mouse cursors, highlight menu items on hover, custom event mouse interaction and much more! See Help.                      id:TDDP_MouseSystemEx
 *
    ...
    */
    ...
    $._precacheCustomCursors = function () {
        if (StorageManager.isLocalMode() && Utils.isOptionValid('test')) {
            ...
            var base = pathmod.dirname(process.mainModule.filename);
            // Find that relative local path, using MV's own methods
            var path = base.replace(/\/[^\/]*$/, '/' + this.customCursorPath);
            if (path.match(/^\/([A-Z]\:)/)) {
                path = path.slice(1);
            }
